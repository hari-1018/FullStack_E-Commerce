...
const Payment = () => {
  ...
  return (
    <div className="bg-gradient-to-r from-pink-100 to-blue-100 p-6 rounded-lg shadow-lg max-w-3xl mx-auto mt-28 mb-28">
      ...
      <ul className="mb-6">
        {cartItems.map((item) => (
          <li key={item.productID._id} className="flex justify-between mb-4 border-b-2 pb-4">
            <div className="flex items-center gap-4">
              <img src={item.productID.image_url} alt={item.productID.name} className="w-16 h-16 object-cover rounded-lg shadow-md" />
              <div>
                <span className="text-md font-semibold text-indigo-600">{item.productID.name} (x{item.quantity})</span>
                <p className="text-sm font-semibold text-grey-400">Price: ₹ {(item.productID.price).toFixed(2)}</p>
              </div>
            </div>
